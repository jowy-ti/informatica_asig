...
void MyGLWidget::ini_camera(){
    ...
    OBS = glm::vec3(VRP.x, VRP.y, VRP.z+2*r);
    up = glm::vec3(0,1,0);
...
void MyGLWidget::calcCapsaContenidora(const Model &model) {
...
void MyGLWidget::calcEsfera() {
    min2 = glm::vec3 (-2.5, 0, -2.5);
    max2 = glm::vec3 (2.5, 1, 2.5);
}
...
void MyGLWidget::viewTransform () {
    ...
    VM = translate (VM, glm::vec3(0., 0., -2*r));
    //VM = glm::rotate(VM, -φ, 0, 0, 1);
    VM = glm::rotate (VM, sy, glm::vec3(1., 0., 0.));
    VM = glm::rotate(VM, sx, glm::vec3(0., 1., 0.));
...
void MyGLWidget::modelTransform2(int i) {
  // Matriu de transformació de model
    
  glm::mat4 transform (1.0f);
  if (i == 0) transform = glm::translate(transform, pos0);
  else if (i == 1) {
      transform = glm::translate(transform, pos1);
      transform = glm::rotate(transform, angle1, glm::vec3(0., 1., 0.));
  }
  else transform = glm::rotate(transform, angle2, glm::vec3(0., 1., 0.));
...
void MyGLWidget::mouseMoveEvent(QMouseEvent *e) {
    ...
    else if(rx>xnew) sx-=0.05;
      
    if(ry<ynew) sy+=0.05;
      
    else if(ry>ynew)sy-=0.05;
